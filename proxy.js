...
export default async function handler(request) {
  ...
  if (apiKey) fwd.set("Api-Key", apiKey);
  const ctIn = request.headers.get("Content-Type");
  if (ctIn) fwd.set("Content-Type", ctIn);
  // ALWAYS spoof the player User-Agent. Browsers always send their own UA, so
  // the old `request UA || VLC` fallback never fired — upstream saw the real
  // browser UA. Many Xtream panels 401/403 desktop-browser UAs on /live and
  // /movie endpoints (anti-restream): phones passed the filter, but Mac and
  // iPad (desktop "Macintosh" UA) were rejected. Verified live on du7g.com:
  // same URL → 401 with a Mac browser UA, 200 with a non-browser UA.
  fwd.set("User-Agent", "VLC/3.0.20 LibVLC/3.0.20");
  ...
  const isManifest =
    /mpegurl|m3u8/i.test(ctype) || /\.m3u8($|\?)/i.test(targetUrl.pathname);
  ...
  if (isManifest) {
    ...
    const wrap = (raw) =>
      proxyBase +
      "?url=" +
      encodeURIComponent(new URL(raw, targetUrl).toString());
